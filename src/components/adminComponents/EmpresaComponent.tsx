...
const EmpresaComponent = () => {
  const { empresas, users, fetchEmpresas, fetchUsers, createEmpresa, editEmpresa } = useAdminStore();
  ...
  useEffect(() => {
    document.body.style.overflow = showModalCrear || showModalEditar || showModalDetalles ? "hidden" : "";
  ...
  const formatDate = (dateString?: string) => {
    ...
    return isNaN(d.getTime()) ? "Fecha inválida" : `${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`;
  };
  ...
  return (
    <div className="p-4 md:p-6 font-['Poppins']">
      ...
      <div className="mb-4 md:mb-6 flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
        ...
        <div className={`${showFilters ? 'flex' : 'hidden'} md:flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4 mt-2 md:mt-0`}>
          <div className="flex items-center gap-2 text-gray-500"><FaFilter />Filtrar por:</div>
          <select value={estado} onChange={(e) => setEstado(e.target.value)} className="px-3 py-2 border rounded-lg cursor-pointer">
            <option value="Todos">Todos</option>
            <option value="Activo">Activo</option>
            <option value="Inactivo">Inactivo</option>
          </select>
          <select value={orden} onChange={(e) => setOrden(e.target.value as "asc" | "desc")} className="px-3 py-2 border rounded-lg cursor-pointer">
            <option value="asc">Fecha Ascendente</option>
            <option value="desc">Fecha Descendente</option>
          </select>
          <button className="bg-green-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 cursor-pointer" onClick={openCreateModal}>
              ...
              .map(e => (
                <tr key={e.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td className="px-3 py-2 md:px-4 md:py-3">
                    <span className={`px-2 py-1 rounded-full text-white text-xs font-semibold ${e.active ? "bg-green-500" : "bg-red-500"}`}>
                      {e.active ? "Activo" : "Inactivo"}
                    </span>
                  </td>
                  <td className="px-3 py-2 md:px-4 md:py-3">{formatDate(e.fecha_alta ?? undefined)}</td>
                  <td className="px-3 py-2 md:px-4 md:py-3">{formatDate(e.ultima_mod ?? undefined)}</td>
                  <td className="px-3 py-2 md:px-4 md:py-3 flex items-center gap-2">
                    <button className="text-[#6d4098] hover:text-[#512e73] cursor-pointer" onClick={() => openEditModal(e)}><FaEdit /></button>
                    <button className="text-[#6d4098] hover:text-[#512e73] cursor-pointer" onClick={() => openDetallesModal(e)}><FaEye /></button>
                  </td>
                </tr>
              ))}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-gray-500">No se encontraron empresas</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {showModalCrear && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <div className="absolute inset-0 bg-black opacity-40" onClick={() => !loading && setShowModalCrear(false)}></div>
          <div className="bg-white rounded-2xl w-full max-w-md mx-4 relative z-10 shadow-lg max-h-[90vh] overflow-y-auto p-4 md:p-6">
            <button onClick={() => !loading && setShowModalCrear(false)} className="absolute top-3 right-3 text-gray-500 hover:text-gray-700 cursor-pointer" disabled={loading}><FaTimes /></button>
            <h3 className="text-lg md:text-xl font-semibold text-[#6d4098] mb-4 md:mb-6 text-center">Crear Empresa</h3>
            ...
            <div className="space-y-3 md:space-y-4">
              ...
              <div>
                ...
                <div className="max-h-40 overflow-y-auto border rounded-md">
                  <select value={condicionIva} onChange={(e) => setCondicionIva(e.target.value)} className="w-full px-3 py-2 text-[#6d4098] focus:outline-none focus:ring-2 focus:ring-[#6d4098] bg-white cursor-pointer" size={5}>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Tipo Societario</label>
                <div className="max-h-40 overflow-y-auto border rounded-md">
                  <select value={tipoSocietario} onChange={(e) => setTipoSocietario(e.target.value)} className="w-full px-3 py-2 text-[#6d4098] focus:outline-none focus:ring-2 focus:ring-[#6d4098] bg-white cursor-pointer" size={5}>
      ...
      {showModalEditar && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <div className="absolute inset-0 bg-black opacity-40" onClick={() => !loading && setShowModalEditar(false)}></div>
          <div className="bg-white rounded-2xl w-full max-w-md mx-4 relative z-10 shadow-lg max-h-[90vh] overflow-y-auto p-4 md:p-6">
            <button onClick={() => !loading && setShowModalEditar(false)} className="absolute top-3 right-3 text-gray-500 hover:text-gray-700 cursor-pointer" disabled={loading}><FaTimes /></button>
            <h3 className="text-lg md:text-xl font-semibold text-[#6d4098] mb-4 md:mb-6 text-center">Editar Empresa</h3>
                ...
                <div className="flex space-x-2 md:space-x-4">
                  <button
                    ...
                    className={`px-4 py-2 md:px-6 md:py-2 rounded-full text-xs md:text-sm font-medium cursor-pointer ${
                      activeEdit ? "bg-green-600 text-white" : "bg-gray-200 text-gray-700"
                    }`}
                  ...
                  <button
                    type="button"
                    className={`px-4 py-2 md:px-6 md:py-2 rounded-full text-xs md:text-sm font-medium cursor-pointer ${
                      !activeEdit ? "bg-red-600 text-white" : "bg-gray-200 text-gray-700"
                    }`}
      ...
      {showModalDetalles && empresaDetalles && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <div className="absolute inset-0 bg-black opacity-40" onClick={() => setShowModalDetalles(false)}></div>
          <div className="bg-white rounded-2xl w-full max-w-2xl mx-4 relative z-10 shadow-lg max-h-[90vh] overflow-y-auto p-4 md:p-6">
            <button onClick={() => setShowModalDetalles(false)} className="absolute top-3 right-3 text-gray-500 hover:text-gray-700 cursor-pointer"><FaTimes /></button>
            <h3 className="text-lg md:text-xl font-semibold text-[#6d4098] mb-4 md:mb-6 text-center">Detalles Completos de la Empresa</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nombre</label>
                <p className="px-3 py-2 rounded-lg bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.nombre}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <p className="px-3 py-2 rounded-lg bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.email}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nombre Fantasía</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.nombre_fantasia}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">CUIT</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.cuit}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Condición IVA</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.condicion_iva}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Tipo Societario</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.tipo_societario}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Actividad Principal</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.actividad_principal || "N/A"}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Domicilio</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.domicilio_legal_calle_numero}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Ciudad</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.domicilio_legal_ciudad}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">País</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.domicilio_legal_pais}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Código Postal</label>
                <p className="px-3 py-2 rounded-lg  bg-opacity-10 text-[#6d4098] font-semibold">{empresaDetalles.codigo_postal}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Estado</label>
                <p className={`px-3 py-2 rounded-lg text-white text-center font-semibold ${empresaDetalles.active ? "bg-green-500" : "bg-red-500"}`}>
                  {empresaDetalles.active ? "ACTIVO" : "INACTIVO"}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Fecha de Alta</label>
                <p className="px-3 py-2 rounded-lg bg-opacity-10 text-[#6d4098] font-semibold">{formatDate(empresaDetalles.fecha_alta ?? undefined)}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Última Modificación</label>
                <p className="px-3 py-2 rounded-lg bg-opacity-10 text-[#6d4098] font-semibold">{formatDate(empresaDetalles.ultima_mod ?? undefined)}</p>
              </div>
            </div>

            <div className="mt-6 flex justify-end">
              <button onClick={() => setShowModalDetalles(false)} className="bg-[#6d4098] text-white px-4 py-2 rounded-lg hover:bg-[#512e73] cursor-pointer">
                Cerrar
              </button>
            </div>
          </div>
